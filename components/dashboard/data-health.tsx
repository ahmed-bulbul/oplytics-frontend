...
type HealthStatus = "synced" | "healthy" | "needs_review" | "missing" | "delayed" | "connected" | "good"
...
> = {
  shopify: {
    ...
    dataIncluded: "Orders, net revenue, refunds, discounts, products, customers",
    usedIn: "Revenue, Net Revenue, AOV, New Customers, Returning Customers, Contribution Margin",
  ...
  meta: {
    ...
    dataIncluded: "Ad spend, impressions, clicks, conversions, campaign + ad set structure",
  ...
  google: {
    ...
    dataIncluded: "Ad spend, impressions, clicks, conversions, campaign structure",
    usedIn: "Ad Spend, ROAS, CPA, MER, Channel Performance",
...
export function DataHealth() {
  ...
  const needsAttention = dataSources.filter(
    (s) => s.status === "needs_review" || s.status === "missing" || s.status === "delayed",
  ).length

  const healthLabel = healthScore >= 90 ? "Good" : healthScore >= 70 ? "Fair" : "Needs Attention"
  const healthColor =
            ...
            <div className="divide-y divide-border">
              {dataSources.map((source) => {
                ...
                return (
                  <div
                    key={source.name}
                    className="grid grid-cols-12 items-center gap-2 px-4 py-2.5 text-xs hover:bg-muted/20"
                  >
                    ...
                    <div className="col-span-5 text-muted-foreground">{source.detail}</div>
                    <div className="col-span-2 flex justify-end">
                      <button
                        onClick={() => openDrawer(source.drawerKind, source.sourceKey)}
            ...
            {dataSources.map((source) => {
              ...
              return (
                <div key={source.name} className="px-4 py-3">
                  ...
                  <p className="mb-1.5 text-[11px] text-muted-foreground">{source.detail}</p>
                  <button
                    onClick={() => openDrawer(source.drawerKind, source.sourceKey)}
                    className="inline-flex items-center gap-1 text-[11px] font-medium text-primary hover:underline"
                  >
      ...
      <Sheet open={drawer.open} onOpenChange={(open) => !open && closeDrawer()}>
        <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-md">
          <VisuallyHidden>
            ...
            <SheetDescription>Review source detail and where it is used.</SheetDescription>
          </VisuallyHidden>

          {drawer.kind === "review_source" && drawer.sourceKey && (
            <ReviewSourceDrawer sourceKey={drawer.sourceKey} onClose={closeDrawer} />
          )}
          {drawer.kind === "review_cogs" && <ReviewCogsDrawer onClose={closeDrawer} />}
          {drawer.kind === "view_logic" && <ViewLogicDrawer onClose={closeDrawer} />}
          {drawer.kind === "view_gaps" && <ViewGapsDrawer onClose={closeDrawer} />}
        </SheetContent>
      </Sheet>
    </div>
  )
...
function ReviewSourceDrawer({ sourceKey, onClose }: { sourceKey: string; onClose: () => void }) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-xs leading-relaxed text-muted-foreground">
        12 products are missing COGS. Contribution Margin may be overstated until these are filled
        in.
      </p>
      ...
      <div className="overflow-hidden rounded-md border border-border">
        ...
        <div className="divide-y divide-border">
          {products.map((p) => (
            <div key={p.name} className="grid grid-cols-12 gap-2 px-3 py-2 text-xs">
              <div className="col-span-6 truncate text-foreground">{p.name}</div>
              <div className="col-span-3 text-right text-muted-foreground">{p.revenue}</div>
...
function ViewLogicDrawer({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-xs leading-relaxed text-muted-foreground">
        Refunds and returns are included in Net Revenue and Contribution Margin calculations.
      </p>
      ...
      <p className="mt-3 text-[11px] text-muted-foreground">Refund data source: Shopify</p>
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-md border border-border">
        ...
        <div className="divide-y divide-border">
          {channels.map((c) => (
            <div key={c.channel} className="grid grid-cols-12 gap-2 px-3 py-2 text-xs">
              <div className="col-span-4 text-foreground">{c.channel}</div>
              <div className="col-span-3 text-right font-medium text-foreground">{c.coverage}</div>
              <div
                className={cn(
                  "col-span-5",
                  c.tone === "good" ? "text-emerald-600" : "text-amber-600",
                )}
              >
                {c.issue}
              </div>
      ...
      <div className="mt-4 rounded-md border border-border bg-muted/20 p-3">
        <p className="text-[11px] leading-relaxed text-foreground">
          Use consistent <span className="font-mono">source</span>,{" "}
          <span className="font-mono">medium</span>, <span className="font-mono">campaign</span>,
          and <span className="font-mono">content</span> naming across paid links.
        </p>
      </div>

      <div className="mt-6 flex flex-wrap gap-2">
        <Button asChild size="sm" variant="outline" className="text-xs">
          <a href="/settings?tab=calculations">View UTM Guidelines</a>
        </Button>
        <Button size="sm" variant="ghost" className="text-xs" onClick={onClose}>
          Close
        </Button>
